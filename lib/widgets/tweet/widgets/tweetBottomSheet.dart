...
class TweetBottomSheet {
  Widget tweetOptionIcon(BuildContext context,
      {required FeedModel model, required TweetType type, required GlobalKey<ScaffoldState> scaffoldKey}) {
    return Container(
      width: 25,
      height: 25,
      ...
      child: customIcon(context, icon: AppIcon.arrowDown, isTwitterIcon: true, iconColor: AppColor.lightGrey),
    ).ripple(
      () {
        _openBottomSheet(context, type: type, model: model, scaffoldKey: scaffoldKey);
      },
  ...
  void _openBottomSheet(BuildContext context,
      {required TweetType type, required FeedModel model, required GlobalKey<ScaffoldState> scaffoldKey}) async {
    ...
    await showModalBottomSheet(
      ...
      builder: (context) {
        return Container(
            ...
            height: context.height * (type == TweetType.Tweet ? (isMyTweet ? .25 : .44) : (isMyTweet ? .38 : .52)),
            ...
            child: type == TweetType.Tweet
                ? _tweetOptions(context, scaffoldKey: scaffoldKey, isMyTweet: isMyTweet, model: model, type: type)
                : _tweetDetailOptions(context, scaffoldKey: scaffoldKey, isMyTweet: isMyTweet, model: model, type: type));
      },
    );
  }
  ...
  Widget _tweetDetailOptions(BuildContext context,
    ...
    return Column(
      children: <Widget>[
        ...
        _widgetBottomSheetRow(context, AppIcon.link, text: 'Copy link to tweet', isEnable: true, onPressed: () async {
          Navigator.pop(context);
          var uri = await Utility.createLinkToShare(
            context,
            "tweet/${model.key}",
            socialMetaTagParameters: SocialMetaTagParameters(
                description: model.description ?? "${model.user!.displayName} posted a tweet on Twitter.",
                title: "Tweet on Twitter app",
                imageUrl: Uri.parse(
                    "https://play-lh.googleusercontent.com/e66XMuvW5hZ7HnFf8R_lcA3TFgkxm0SuyaMsBs3KENijNHZlogUAjxeu9COqsejV5w=s180-rw")),
          );

          Utility.copyToClipBoard(context: context, text: uri.toString(), message: "Tweet link copy to clipboard");
        }),
        isMyTweet
        ...
        child: Row(
          children: <Widget>[
            customIcon(
              ...
              size: 25,
              paddingIcon: 8,
              iconColor: onPressed != null ? AppColor.darkGrey : AppColor.lightGrey,
            ),
            const SizedBox(
              width: 15,
            ),
  ...
  void _deleteTweet(BuildContext context, TweetType type, String tweetId, {String? parentkey}) {
  ...
  void openRetweetBottomSheet(BuildContext context,
      {TweetType? type, required FeedModel model, required GlobalKey<ScaffoldState> scaffoldKey}) async {
    await showModalBottomSheet(
      backgroundColor: Colors.transparent,
      context: context,
      builder: (context) {
  ...
  void openShareTweetBottomSheet(BuildContext context, FeedModel model, TweetType? type) async {
    await showModalBottomSheet(
      backgroundColor: Colors.transparent,
      context: context,
      builder: (context) {
  ...
  Widget _shareTweet(BuildContext context, FeedModel model, TweetType? type) {
    var socialMetaTagParameters = SocialMetaTagParameters(
        ...
        title: "${model.user!.displayName} posted a tweet on Twitter.",
            ...
            Navigator.push(
              ...
              ShareWidget.getRoute(
                  child: type != null
                      ...
                      : Tweet(model: model, scaffoldKey: GlobalKey<ScaffoldState>()),
                  id: "tweet/${model.key}",
                  socialMetaTagParameters: socialMetaTagParameters),
            );
          },
