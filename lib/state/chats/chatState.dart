...
class ChatState extends AppState {
  ...
  List<ChatMessage>? get messageList {
    if (_messageList == null) {
      ...
      _messageList!.sort((x, y) => DateTime.parse(y.createdAt!).toLocal().compareTo(DateTime.parse(x.createdAt!).toLocal()));
      return _messageList;
    }
  }
  ...
  void databaseInit(String userId, String myId) async {
    ...
    kDatabase.child("chatUsers").child(myId).onChildAdded.listen(_onChatUserAdded);
  ...
  void getUserChatList(String userId) {
    try {
      kDatabase.child('chatUsers').child(userId).once().then((DatabaseEvent event) {
        ...
        if (snapshot.value != null) {
          ...
          _chatUserList!.sort((x, y) {
            if (x.createdAt != null && y.createdAt != null) {
              return DateTime.parse(y.createdAt!).compareTo(DateTime.parse(x.createdAt!));
            } else {
              if (x.createdAt != null) {
                return 0;
              } else {
  ...
  void getChatDetailAsync() async {
    try {
      kDatabase.child('chats').child(_channelName!).once().then((DatabaseEvent event) {
        final snapshot = event.snapshot;
  ...
  void onMessageSubmitted(
    ...
    try {
      ...
      kDatabase.child('chatUsers').child(message.senderId).child(message.receiverId).set(message.toJson());

      kDatabase.child('chatUsers').child(chatUser!.userId!).child(message.senderId).set(message.toJson());
  ...
  void _onMessageAdded(DatabaseEvent event) {
    ...
    if (event.snapshot.value != null) {
      ...
      if (map != null) {
        ...
        if (_messageList!.isNotEmpty && _messageList!.any((x) => x.key == model.key)) {
          return;
        }
        _messageList!.add(model);
      }
  ...
  void _onChatUserAdded(DatabaseEvent event) {
    ...
    if (event.snapshot.value != null) {
      ...
      if (map != null) {
        var model = ChatMessage.fromJson(map);
        model.key = event.snapshot.key!;
        if (_chatUserList!.isNotEmpty && _chatUserList!.any((x) => x.key == model.key)) {
          return;
        }
        _chatUserList!.add(model);
      }
  ...
  void sendAndRetrieveMessage(ChatMessage model) async {
    ...
    var body = jsonEncode(<String, dynamic>{
      'notification': <String, dynamic>{'body': model.message, 'title': "Message from ${model.senderName}"},
      'priority': 'high',
      'data': <String, dynamic>{
