...
class BookmarkState extends AppState {
  ...
  void getDataFromDatabase() async {
    String userId = await getIt<SharedPreferenceHelper>().getUserProfile().then((value) => value!.userId!);
    try {
      if (_tweetList != null) {
        return;
      }
      isBusy = true;
      kDatabase.child('bookmark').child(userId).once().then((DatabaseEvent event) async {
